...
class ChaseTheDotEnv(gym.Env):
                    ...
                    if buf[18:20] == b"\r\n":
                        ...
                        if self.prev_gx is not None:
                            ...
                            if jump > 40.0:
                                ...
                                self.v_smooth_x = 0.8 * self.v_smooth_x + 0.2 * dx
                                self.v_smooth_y = 0.8 * self.v_smooth_y + 0.2 * dy
                                if self.prev_dx is not None:
                                    ax = dx - self.prev_dx
                                    ay = dy - self.prev_dy
                                    self.a_smooth_x = 0.8 * self.a_smooth_x + 0.2 * ax
                                    self.a_smooth_y = 0.8 * self.a_smooth_y + 0.2 * ay
                                self.prev_dx, self.prev_dy = dx, dy
                        self.prev_gx, self.prev_gy = gx, gy
                        self.error_x, self.error_y = bool(ex), bool(ey)
                        self._latest = np.array([gx, gy, bx, by, self.v_smooth_x, self.v_smooth_y, self.a_smooth_x, self.a_smooth_y], dtype=np.float32)
                        del buf[:20]
                        self._new_data.set()
                    else:
                        idx = buf.find(b"\r\n")
    ...
    def _metrics(self, s: np.ndarray) -> Tuple[np.ndarray, float, dict]:
        ...
        in_bounds = not (self.error_x or self.error_y)
    ...
    def step(self, action, wait_for_new: bool = True) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        # P_command = P_Green + v_smooth + 0.5 * a_smooth + offset + action * action_scale
        x = int(np.clip(round(gx + self.v_smooth_x + 0.5 * self.a_smooth_x + offset + float(action[0]) * self.action_scale), -50, 950))
        y = int(np.clip(round(gy + self.v_smooth_y + 0.5 * self.a_smooth_y + offset + float(action[1]) * self.action_scale), -50, 950))
        if self.socket:
